...
class InstancedLOD {
  ...
  render() {
    count++
    let {
      instancedMeshOfAllLevel, groupOfInstances, numOfLevel, camera, frustum, octree, worldProjectionMatrix, obj_position, cur_dist, cur_level
    } = this;
    // clear
    ...
    for (let i = 0; i < numOfLevel; i++) {
      ...
      for (let j = 0; j < groupOfInstances[i].children.length; j++) {
        ...
        // instancedMesh.instanceMatrix = new THREE.InstancedBufferAttribute(new Float32Array(obj.matrix4), 16);
